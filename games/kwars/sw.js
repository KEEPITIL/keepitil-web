...
self.addEventListener('activate', e => {
  e.waitUntil(
    caches.keys()
      // `kwars1-*` is the RETIRED route's namespace and is always obsolete now;
      // `kwars-*` is this game's own namespace, so only older versions of it go.
      // Nothing outside these prefixes is touched, and caches.delete() never
      // reaches localStorage / IndexedDB, so saves and auth are untouched.
      .then(keys => Promise.all(keys.filter(k => (
             k.indexOf('kwars1-')===0 ||
             (k.indexOf('kwars-')===0 && k !== CACHE) ||
             k.indexOf('skw-')===0 || k.indexOf('kingdom-wars')===0 || k.indexOf('ageofwars')===0
