...
(function () {
  ...
  var CSS = ''
   ...
   /* ⚠ THE ROW MUST NOT RE-CENTRE ITSELF INSIDE THE BAR (Founder 2026-09-06).
      This capped the row at 1120 and centred it with margin:auto, so inside a bar already
      placed by the canonical grid the row sat 80px further in and the first control did not
      line up with the first card - measured rowLeft 160 against barLeft 80. The bar owns the
      page position; the row simply fills it. Mobile re-applies its own 16px below. */
   + '#vs-app .ce-row{max-width:none;margin-left:0;margin-right:0;padding-left:0;padding-right:0;width:100%}'
   ...
   /* The two dropdown-style controls. A real <select> so the options are native and
      accessible; styled to sit in the chip rail rather than look like a form field. */
   + '#vs-app .ce-dd{flex:0 0 auto;display:inline-flex}'
   + '#vs-app .ce-sel{-webkit-appearance:none;appearance:none;height:30px;padding:0 26px 0 11px;'
   +   'border-radius:5px;cursor:pointer;background:transparent;color:#c8d0da;'
   +   'border:1px solid rgba(255,255,255,.26);font:700 11px Inter,sans-serif;'
   +   'letter-spacing:.08em;text-transform:uppercase;'
   +   'background-image:linear-gradient(45deg,transparent 50%,#8b95a3 50%),'
   +     'linear-gradient(135deg,#8b95a3 50%,transparent 50%);'
   +   'background-position:calc(100% - 14px) 13px,calc(100% - 9px) 13px;'
   +   'background-size:5px 5px,5px 5px;background-repeat:no-repeat}'
   + '#vs-app .ce-sel option{background:#12121a;color:#e8ecf1;text-transform:none;letter-spacing:0}'
   /* ── THE FILTER ROW FILLS ITS SECTION, LIKE DISCOVER (Founder 2026-09-06) ──────────
      DISCOVER's model, measured: the dropdown is flex:0 0 auto, the month wrapper is
      flex:1 1 auto so it claims everything left over, and each month is flex:1 1 0 so they
      come out equal and reach the right edge. CREATE sized every chip to its own label, so
      row 1 stopped at 1143 inside a 1360 bar - the ~215px of dead space at the end.
      Desktop only: below 861px both rows stay horizontally scrollable rails, which is the
      approved mobile behaviour and must not become a squeeze. */
   + '@media(min-width:861px){'
   +   '#vs-app .ce-mwrap{flex:1 1 auto;min-width:0;display:flex;gap:6px}'
   +   '#vs-app .ce-mwrap .ce-chip{flex:1 1 0;min-width:0;padding:0 3px;'
   +     'overflow:hidden;text-overflow:ellipsis;white-space:nowrap;justify-content:center;'
   /* Slightly tighter tracking here than the base chip: at twelve equal columns the only
      label that did not fit was the month carrying a count ("DECEMBER 7", 95px into 88px).
      Every plain month name fits; this buys the count its few pixels rather than dropping
      to abbreviations, which the brief explicitly asked to avoid. */
   +     'letter-spacing:.04em}'
   +   '#vs-app .ce-mwrap .ce-chip i{margin-left:3px}'
   /* The longest label, DECEMBER with its count, measured 78px into exactly 78px of room -
      fitting, but with nothing spare, so any font substitution would start clipping it.
      Trimming the side padding leaves a few pixels of headroom instead of relying on luck. */
   /* Both dropdowns share one width so they read as a matched pair rather than two
      controls that happen to sit above each other. Wide enough for UPCOMING, the longest
      label either of them carries. */
   +   '#vs-app .ce-dd{flex:0 0 auto}'
   +   '#vs-app .ce-sel{width:132px}'
   + '}'
   /* Month headings give CREATE the same chronological hierarchy DISCOVER has. */
   + '#vs-app .ce-mgroup{margin:0 0 26px}'
   + '#vs-app .ce-mhead{font-family:var(--font-head,inherit);font-size:clamp(1.7rem,3.4vw,2.6rem);'
   +   'line-height:1;letter-spacing:.06em;text-transform:uppercase;color:var(--vsb,#00b4ff);'
   +   'margin:0 0 12px;padding:0}'
   + '@media(max-width:860px){#vs-app .ce-mhead{font-size:1.5rem;margin:0 0 9px;padding-left:16px}'
   +   '#vs-app .ce-mgroup{margin:0 0 18px}}'
   ...
   /* ⚠ SCROLL SNAP IGNORES THE LEADING PADDING UNLESS scroll-padding SAYS OTHERWISE.
      The 16px gutter above was applied and computed correctly, and the rail still rendered
      its first card flush at x=0: scroll-snap-align:start aligns the card to the SCROLLPORT
      edge, so the rail simply scrolled 16px past its own padding (measured scrollLeft:16).
      The DOM reported the gutter while the screen did not have one. scroll-padding moves the
      snapport in to match, so the first card rests on the gutter the chips already use. */
   +   '#vs-app .ce-grid{scroll-padding-left:16px}'
   ...
   /* .wrap gives up its padding on mobile (create/index.html), so the bar spans the screen
      edge to edge while each section re-applies the SAME 16px the cards use. The chips are
      inset by the row's own padding, so the first chip still starts on the card edge and the
      rail scrolls the full width behind it. One gutter, declared once, no breakout. */
   +   '#vs-app .ce-grid,#vs-app .ce-hero,#vs-app .ce-stats{padding-left:16px;padding-right:16px}'
   +   '#vs-app .ce-row{padding-left:16px;padding-right:16px}'
   +   '#vs-app .ce-row{flex-wrap:nowrap;overflow-x:auto;scrollbar-width:none;min-width:0}'
   +   '#vs-app .ce-row::-webkit-scrollbar{display:none}'
   /* ⚠ THE DESKTOP WRAPPER MUST NOT EXIST ON MOBILE. .ce-mwrap groups the chips so they can
      share the row on desktop; on a phone the row is a horizontal scroll rail, and a block
      wrapper inside it holds 920px of chips in 270px - they wrapped and squeezed instead of
      scrolling, and four chips fell under 60px. display:contents promotes the chips back to
      being direct children of the scrolling row, which is exactly the approved behaviour. */
   +   '#vs-app .ce-mwrap{display:contents}'
  ...
  function renderMine(){
    if(!ME){ APP.innerHTML = navBar('mine') + '<div class="soon"><div class="i">🔐</div><h2>Sign in</h2><p>Your VS entries live here.</p><a class="vs-cta" href="/apply">Sign in</a></div>'; return; }
  ...
  function renderVotes(){
    if(!ME){ APP.innerHTML = navBar('votes') + '<div class="soon"><div class="i">🔐</div><h2>Sign in</h2><p>Your voting history lives here.</p><a class="vs-cta" href="/apply">Sign in</a></div>'; return; }
  ...
  var CE_MONTH='all', CE_TYPE='all', CE_YEAR='all', CE_SORT='upcoming', CE_ROWS=null;

  function ceMonthKey(c){ return c.submissions_close_at ? String(c.submissions_close_at).slice(0,7) : ''; }
  function ceMonthLabel(mk){
    /* Written out in full (Founder 2026-09-06). The row is a known twelve-month window, so
       the names have room and the abbreviations only made it harder to scan. */
    var M=['JANUARY','FEBRUARY','MARCH','APRIL','MAY','JUNE',
           'JULY','AUGUST','SEPTEMBER','OCTOBER','NOVEMBER','DECEMBER'];
    return M[parseInt(mk.slice(5,7),10)-1];
  }
  /* The years the data actually offers, newest first, so the dropdown never lists an empty one. */
  function ceYears(rows){
    var seen={};
    (rows||[]).forEach(function(c){ var mk=ceMonthKey(c); if(mk) seen[mk.slice(0,4)]=1; });
  ...
  function ceMonthWindow(){
    /* ALL keeps the rolling twelve from the current month; picking a specific year shows that
       year's calendar instead, because "rolling into next year" stops meaning anything once a
       year is chosen. */
    if(CE_YEAR!=='all'){
      var out2=[]; for(var k=1;k<=12;k++) out2.push(CE_YEAR+'-'+String(k).padStart(2,'0'));
      return out2;
    }
    var d=new Date(), out=[];
    for(var i=0;i<12;i++){
  ...
  /* ── EVENT STATE (Founder 2026-09-06) ───────────────────────────────────────────────
     UPCOMING / PAST / VOTE select WHICH events are shown; the two ORDER modes only reorder
     what is already there. They share one control because the visitor is answering one
     question - what am I looking at, and in what order - and the default is UPCOMING. */
  function ceIsUpcoming(c){
    var t=c.submissions_close_at; if(!t) return true;      /* no deadline: still open */
    return new Date(t).getTime() >= Date.now();
  }
  function ceIsPast(c){
    var t=c.submissions_close_at; if(!t) return false;
    return new Date(t).getTime() < Date.now();
  }
  function ceIsVoting(c){
    if(c.voting_open === true) return true;
    var o=c.voting_opens_at, x=c.voting_closes_at, now=Date.now();
    if(!o && !x) return false;
    if(o && new Date(o).getTime() > now) return false;
    if(x && new Date(x).getTime() < now) return false;
    return true;
  }
  var CE_SORTS=[
    ...
    {k:'past',     label:'PAST'},
    {k:'vote',     label:'VOTE'},
    {k:'az',       label:'ORDER A\u2013Z'},
    {k:'za',       label:'ORDER Z\u2013A'}
  ];
  function ceSortLabel(k){
    for(var i=0;i<CE_SORTS.length;i++) if(CE_SORTS[i].k===k) return CE_SORTS[i].label;
    return 'UPCOMING';
  }
  function ceMatch(c){
    if(CE_MONTH!=='all' && ceMonthKey(c)!==CE_MONTH) return false;
    if(CE_TYPE!=='all'  && (c.category||'')!==CE_TYPE) return false;
    if(CE_YEAR!=='all'  && ceMonthKey(c).slice(0,4)!==CE_YEAR) return false;
    if(CE_SORT==='upcoming' && !ceIsUpcoming(c)) return false;
    if(CE_SORT==='past'     && !ceIsPast(c))     return false;
    if(CE_SORT==='vote'     && !ceIsVoting(c))   return false;
    return true;
  }
  ...
  function ceRender(){
    ...
    var shown=rows.filter(ceMatch).slice().sort(function(a,b){
      if(CE_SORT==='az' || CE_SORT==='za'){
        var ta=String(a.title||''), tb=String(b.title||'');
        var c=ta.localeCompare(tb, undefined, {sensitivity:'base'});
        return CE_SORT==='za' ? -c : c;
      }
      var A=a.submissions_close_at||'', B=b.submissions_close_at||'';
      if(!A && !B) return 0;
      if(!A) return 1;
      if(!B) return -1;
      /* PAST reads newest-first: the thing that just closed matters more than the oldest one. */
      if(CE_SORT==='past') return A < B ? 1 : A > B ? -1 : 0;
    ...
    var html='<div class="ce-bar kil-filter-shell">'
      + '<div class="ce-row">'
        ...
        /* ROW 1 - YEAR then the month window. The year is a dropdown-style control and
           defaults to ALL; the months to its right follow whatever it selects. */
        + '<span class="ce-dd">'
        +   '<select class="ce-chip ce-sel" id="ceYear" aria-label="Year">'
        +     '<option value="all"'+(CE_YEAR==='all'?' selected':'')+'>ALL</option>'
        +     yKeys.map(function(y){
                return '<option value="'+h(y)+'"'+(CE_YEAR===y?' selected':'')+'>'+h(y)+'</option>'; }).join('')
        +   '</select>'
        + '</span>'
        /* The months live in their own flex box so they can share whatever the dropdown
           leaves, the way DISCOVER's .evx-monthwrap does. Without it they sized to their
           labels and the row stopped ~215px short of the right edge. */
        + '<span class="ce-mwrap">'
        ...
        + '</span>'
      + '</div>'
      /* ROW 2 - event state / order, then the categories, which are kept rather than
         absorbed: the state control answers "which events", the categories "of what kind". */
      + '<div class="ce-row">'
        + '<span class="ce-dd">'
        +   '<select class="ce-chip ce-sel" id="ceSort" aria-label="Event state and order">'
        +     CE_SORTS.map(function(o){
                return '<option value="'+o.k+'"'+(CE_SORT===o.k?' selected':'')+'>'+o.label+'</option>'; }).join('')
        +   '</select>'
        + '</span>'
        /* The TYPE label is gone (Founder 2026-09-06). It was a chip that named the row
           rather than filtering anything, and the row already reads as categories. Clearing
           the filter is not lost with it: clicking the active category toggles it off, so
           the reset lives on the control the visitor is already pointing at. */
        + '<span class="ce-mwrap">'
    ...
    /* GROUPED BY MONTH (Founder 2026-09-06), the same chronological hierarchy DISCOVER
       uses, instead of one undifferentiated grid. A–Z / Z–A are deliberately NOT grouped:
       an alphabetical list broken up by month is neither one ordering nor the other.
       Anything without a deadline collects under its own heading rather than inventing a
       month for itself. */
    function ceGrouped(list){
      var order=[], bucket={};
      list.forEach(function(c){
        var mk=ceMonthKey(c)||'none';
        if(!bucket[mk]){ bucket[mk]=[]; order.push(mk); }
        bucket[mk].push(c);
      });
      return order.map(function(mk){
        var head = mk==='none' ? 'NO DEADLINE'
                 : ceMonthLabel(mk)+' '+mk.slice(0,4);
        return '<div class="ce-mgroup">'
             +   '<h2 class="ce-mhead">'+h(head)+'</h2>'
             +   '<div class="ce-grid">'+bucket[mk].map(ceCard).join('')+'</div>'
             + '</div>';
      }).join('');
    }
    html += shown.length
      ? ((CE_SORT==='az'||CE_SORT==='za')
    ...
    var ySel=APP.querySelector('#ceYear');
    if(ySel) ySel.onchange=function(){
      CE_YEAR=ySel.value;
      /* A month from the previous window would filter everything out silently. */
      CE_MONTH='all';
      ceRender();
    };
    var sSel=APP.querySelector('#ceSort');
    if(sSel) sSel.onchange=function(){ CE_SORT=sSel.value; ceRender(); };
    APP.querySelectorAll('[data-t]').forEach(function(b){
      b.onclick=function(){
        /* Clicking the category that is already on clears it - this is what replaces the
           removed TYPE chip as the way back to "all". */
        CE_TYPE = (CE_TYPE===b.dataset.t) ? 'all' : b.dataset.t;
        ceRender();
      };
    });
  ...
  function renderEntryForm(compId){
    ...
    if(!ME){
      APP.innerHTML = navBar('enter')
        ...
        + '<a class="vs-cta" href="/apply">Sign in</a>'
