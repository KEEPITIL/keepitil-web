(function(){
  ...
  let data=load();
  /* SCREEN SHAKE: off by default for everyone.
     The setting existed before but the renderer never read it, so no player has
     ever actually chosen "on" -- an existing save carrying shake:true recorded a
     default, not a preference. Migrate those once to off. The flag makes the
     migration idempotent, so a player who later turns shake on keeps it. */
  /* NOTE: shakeMigrated must NOT be part of `base.settings`. merge() spreads
     base first, so a default flag there would be present on every legacy save
     and the migration would never run. Its absence is exactly the signal. */
  if(!data.settings.shakeMigrated){ data.settings.shake=false; data.settings.shakeMigrated=true; }
  if(typeof data.settings.shakeIntensity!=='number'||!isFinite(data.settings.shakeIntensity))
    data.settings.shakeIntensity=60;
  data.settings.shakeIntensity=Math.max(10,Math.min(100,Math.round(data.settings.shakeIntensity)));
  /* The single place the renderer asks "how much may I shake the camera?".
     0 means absolutely none -- no battlefield event can move the camera. */
  window.KWShakeScale=()=>data.settings.shake?(data.settings.shakeIntensity/100):0;
